...
public class BackpackRenderInfo {
    ...
    public Optional<Pair<FluidStack, FluidStack>> getTanksContents() {
        if(this.backpack.has(ModDataComponents.UPGRADES)) {
            Optional<ItemStack> tanksUpgrade = this.backpack.get(ModDataComponents.UPGRADES).getItems().stream().filter(stack -> stack.getItem() == ModItemsNeo.TANKS_UPGRADE.get()).findFirst();
    ...
    public boolean isDyed() {
        return this.backpack.has(DataComponents.DYED_COLOR) && this.backpack.getItem() == ModItemsNeo.STANDARD_TRAVELERS_BACKPACK.get();
    }
