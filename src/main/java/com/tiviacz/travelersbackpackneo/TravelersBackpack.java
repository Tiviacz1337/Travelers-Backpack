...
public class TravelersBackpack {
    ...
    public TravelersBackpack(IEventBus eventBus, ModContainer modContainer) {
        ...
        ModItemsNeo.ITEMS.register(eventBus);
        ModItemsNeo.ENTITY_TYPES.register(eventBus);
    ...
    public static void enableCraftingTweaks() {
        if(craftingTweaksLoaded) {
            try {
                Class.forName("com.tiviacz.travelersbackpackneo.compat.craftingtweaks.TravelersBackpackCraftingGridProvider").getConstructor().newInstance();
            } catch(Throwable e) {
                e.printStackTrace();
            }
        }
