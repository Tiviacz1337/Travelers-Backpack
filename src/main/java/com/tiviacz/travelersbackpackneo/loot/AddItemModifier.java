package com.tiviacz.travelersbackpackneo.loot;
...
public class AddItemModifier extends LootModifier {
    ...
    protected @NotNull ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        ...
        if(context.getQueriedLootTableId().equals(BuiltInLootTables.ABANDONED_MINESHAFT.location())) {
            if(this.item == ModItemsNeo.BAT_TRAVELERS_BACKPACK.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(withTanksUpgrade(item));
            }

            if(this.item == ModItemsNeo.STANDARD_TRAVELERS_BACKPACK.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(withTanksUpgrade(item));
            }

            if(this.item == ModItemsNeo.IRON_TIER_UPGRADE.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(new ItemStack(item));
            }

            if(this.item == ModItemsNeo.GOLD_TIER_UPGRADE.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(new ItemStack(item));
            }
        }

        if(context.getQueriedLootTableId().equals(BuiltInLootTables.VILLAGE_ARMORER.location())) {
            if(item == ModItemsNeo.IRON_GOLEM_TRAVELERS_BACKPACK.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(withTanksUpgrade(item));
            }
        }

        if(context.getQueriedLootTableId().equals(BuiltInLootTables.SIMPLE_DUNGEON.location())) {
            if(item == ModItemsNeo.STANDARD_TRAVELERS_BACKPACK.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(withTanksUpgrade(item));
            }
        ...
        if(context.getQueriedLootTableId().equals(BuiltInLootTables.END_CITY_TREASURE.location())) {
            ...
            if(this.item == ModItemsNeo.DIAMOND_TIER_UPGRADE.get() && context.getRandom().nextFloat() <= this.weight) {
                generatedLoot.add(new ItemStack(item));
            }
        }
