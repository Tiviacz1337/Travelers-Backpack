...
import com.tiviacz.travelersbackpackold.common.BackpackAbilities;
...
import com.tiviacz.travelersbackpackold.inventory.screen.slot.ToolSlot;
import com.tiviacz.travelersbackpackold.inventory.sorter.SlotManager;
import com.tiviacz.travelersbackpackold.inventory.sorter.wrappers.CombinedInvWrapper;
import com.tiviacz.travelersbackpackold.network.SyncItemStackPacket;
...
public class TravelersBackpackInventory implements ITravelersBackpackInventory
{
    ...
    {
        this.stack.set(ModDataComponents.BACKPACK_CONTAINER, itemsToList(this.stack.contains(ModDataComponents.BACKPACK_CONTAINER) ? this.inventory.size() : this.tier.getStorageSlots(), this.inventory));
        this.stack.set(ModDataComponents.CRAFTING_CONTAINER, itemsToList(9, this.craftingInventory));
        this.stack.set(ModDataComponents.TOOLS_CONTAINER, itemsToList(this.stack.contains(ModDataComponents.TOOLS_CONTAINER) ? this.toolSlots.size() : this.tier.getToolSlots(), this.toolSlots));
    }

    public void readItems()
    {
        this.inventory = createInventory(this.stack.getOrDefault(ModDataComponents.BACKPACK_CONTAINER, BackpackContainerComponent.fromStacks(this.tier.getStorageSlots(), DefaultedList.ofSize(this.tier.getStorageSlots(), ItemStack.EMPTY))).getStacks(), true);
        this.toolSlots = createToolsInventory(this.stack.getOrDefault(ModDataComponents.TOOLS_CONTAINER, BackpackContainerComponent.fromStacks(this.tier.getToolSlots(), DefaultedList.ofSize(this.tier.getToolSlots(), ItemStack.EMPTY))).getStacks());
        this.craftingInventory = createInventory(this.stack.getOrDefault(ModDataComponents.CRAFTING_CONTAINER, BackpackContainerComponent.fromStacks(9, DefaultedList.ofSize(9, ItemStack.EMPTY))).getStacks(), false);
    ...
    {
        this.stack.set(ModDataComponents.FLUID_TANKS, new FluidTanks(this.leftTank.getCapacity(), new FluidTanks.Tank(this.leftTank.getResource(), this.leftTank.getAmount()), new FluidTanks.Tank(this.rightTank.getResource(), this.rightTank.getAmount())));
    }

    public void readTanks()
    {
        FluidTanks tanks = this.stack.getOrDefault(ModDataComponents.FLUID_TANKS, FluidTanks.createTanks(this.tier.getTankCapacity()));
    ...
    {
        this.stack.set(ModDataComponents.ABILITY_SWITCH, this.ability);
    }

    public void readAbility()
    {
        this.ability = this.stack.getOrDefault(ModDataComponents.ABILITY_SWITCH, TravelersBackpackConfig.getConfig().backpackAbilities.forceAbilityEnabled);
    }

    public void writeTime()
    {
        this.stack.set(ModDataComponents.LAST_TIME, this.lastTime);
    }

    public void readTime()
    {
        this.lastTime = this.stack.getOrDefault(ModDataComponents.LAST_TIME, 0);
    }
    ...
    {
        return this.stack.contains(ModDataComponents.SLEEPING_BAG_COLOR);
    }
    ...
    public void markSlotDirty(int index, ItemStack stack, byte dataId)
    {
        switch(dataId)
        {
            case INVENTORY_DATA: this.stack.apply(ModDataComponents.BACKPACK_CONTAINER, new BackpackContainerComponent(this.getTier().getStorageSlots()), new BackpackContainerComponent.Slot(index, stack), BackpackContainerComponent::updateSlot); break;
            case CRAFTING_INVENTORY_DATA: this.stack.apply(ModDataComponents.CRAFTING_CONTAINER, new BackpackContainerComponent(9), new BackpackContainerComponent.Slot(index, stack), BackpackContainerComponent::updateSlot); break;
            case TOOLS_DATA: this.stack.apply(ModDataComponents.TOOLS_CONTAINER, new BackpackContainerComponent(this.tier.getToolSlots()), new BackpackContainerComponent.Slot(index, stack), BackpackContainerComponent::updateSlot); break;
        }
        sendPackets();
    }
