...
import com.tiviacz.travelersbackpackold.mixin.KeybindingAccessor;
import com.tiviacz.travelersbackpackold.network.AbilitySliderPacket;
import com.tiviacz.travelersbackpackold.network.SpecialActionPacket;
...
public class KeybindHandler {
    ...
    public static void registerListeners() {
        ClientTickEvents.START_CLIENT_TICK.register(evt -> {
            ...
            if (ComponentUtils.isWearingBackpack(player)) {
                ...
                if (player.getMainHandStack().getItem() instanceof HoseItem && player.getMainHandStack().contains(ModDataComponents.HOSE_MODES)) {
                ...
                if (TravelersBackpackConfig.getConfig().client.disableScrollWheel) {
                    ...
                    while (SWITCH_TOOL.wasPressed()) {
                        if (!heldItem.isEmpty()) {
                            if (heldItem.getItem() instanceof HoseItem && heldItem.contains(ModDataComponents.HOSE_MODES)) {
    ...
    public static boolean onMouseScroll(double deltaX, double deltaY) {
        ...
        if (!TravelersBackpackConfig.getConfig().client.disableScrollWheel && deltaY != 0.0) {
            ...
            if (player != null && player.isAlive() && InputUtil.isKeyPressed(mc.getWindow().getHandle(), ((KeybindingAccessor)KeybindHandler.SWITCH_TOOL).getBoundKey().getCode())) { //KeybindHandler.SWITCH_TOOL.isPressed()) {//&& KeybindHandler.SWITCH_TOOL.isPressed()) {
                if (ComponentUtils.isWearingBackpack(player)) {
                    ItemStack heldItem = player.getMainHandStack();
                    if(!heldItem.isEmpty()) {
                        if (heldItem.getItem() instanceof HoseItem && heldItem.contains(ModDataComponents.HOSE_MODES)) {
