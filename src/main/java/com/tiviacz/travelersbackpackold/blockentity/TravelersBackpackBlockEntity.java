package com.tiviacz.travelersbackpackold.blockentity;
...
import com.tiviacz.travelersbackpackold.inventory.*;
...
public class TravelersBackpackBlockEntity extends BlockEntity implements ITravelersBackpackInventory, Nameable
{
    ...
    protected void readComponents(ComponentsAccess components)
    {
        ...
        this.tier = Tiers.of(components.getOrDefault(ModDataComponents.TIER, 0));
        this.inventory = createInventory(components.getOrDefault(ModDataComponents.BACKPACK_CONTAINER, BackpackContainerComponent.fromStacks(this.tier.getStorageSlots(), DefaultedList.ofSize(this.tier.getStorageSlots(), ItemStack.EMPTY))).getStacks(), true);
        this.toolSlots = createToolsInventory(components.getOrDefault(ModDataComponents.TOOLS_CONTAINER, BackpackContainerComponent.fromStacks(this.tier.getToolSlots(), DefaultedList.ofSize(this.tier.getToolSlots(), ItemStack.EMPTY))).getStacks());
        this.craftingInventory = createInventory(components.getOrDefault(ModDataComponents.CRAFTING_CONTAINER, BackpackContainerComponent.fromStacks(9, DefaultedList.ofSize(9, ItemStack.EMPTY))).getStacks(), false);

        //Fluid Tanks
        FluidTanks tanks = components.getOrDefault(ModDataComponents.FLUID_TANKS, FluidTanks.createTanks(this.tier.getTankCapacity()));
        ...
        this.sleepingBagColor = components.getOrDefault(ModDataComponents.SLEEPING_BAG_COLOR, DyeColor.RED.getId());
        this.settingsManager = settingsManager.getManager(components.getOrDefault(ModDataComponents.SETTINGS, settingsManager.createDefaults()));
        this.slotManager = slotManager.getManager(components.getOrDefault(ModDataComponents.SLOTS, Slots.createDefault()));
        this.lastTime = components.getOrDefault(ModDataComponents.LAST_TIME, 0);
    ...
    protected void addComponents(ComponentMap.Builder componentMapBuilder)
    {
        ...
        componentMapBuilder.add(ModDataComponents.TIER, this.tier.getOrdinal());

        if(!this.inventory.isEmpty())
        {
            componentMapBuilder.add(ModDataComponents.BACKPACK_CONTAINER, itemsToList(this.inventory.size(), this.inventory.getStacks()));
        }
        if(!this.toolSlots.isEmpty())
        {
            componentMapBuilder.add(ModDataComponents.TOOLS_CONTAINER, itemsToList(this.toolSlots.size(), this.toolSlots.getStacks()));
        }
        if(!this.craftingInventory.isEmpty())
        {
            componentMapBuilder.add(ModDataComponents.CRAFTING_CONTAINER, itemsToList(this.craftingInventory.size(), this.craftingInventory.getStacks()));
        }
        if(!this.leftTank.isResourceBlank() || !this.rightTank.isResourceBlank())
        {
            ...
            componentMapBuilder.add(ModDataComponents.FLUID_TANKS, new FluidTanks(this.leftTank.getCapacity(), new FluidTanks.Tank(this.leftTank.getResource(), this.leftTank.getAmount()), new FluidTanks.Tank(this.rightTank.getResource(), this.rightTank.getAmount())));
        }
        if(this.hasColor())
        {
        ...
        {
            componentMapBuilder.add(ModDataComponents.SLEEPING_BAG_COLOR, this.sleepingBagColor);
        }

        if(!this.settingsManager.isDefault())
        {
            componentMapBuilder.add(ModDataComponents.SETTINGS, this.settingsManager.getSettings());
        }

        if(!this.slotManager.getMemorySlots().isEmpty() || !this.slotManager.getUnsortableSlots().isEmpty())
        {
            componentMapBuilder.add(ModDataComponents.SLOTS, this.slotManager.getSlots());
        }

        if(this.lastTime != 0)
        {
            componentMapBuilder.add(ModDataComponents.LAST_TIME, this.lastTime);
        }
