...
import com.tiviacz.travelersbackpackold.entity.BackpackItemEntity;
...
public class TravelersBackpackItem extends BlockItem
{
    ...
    {
        if(stack.contains(ModDataComponents.TIER))
        {
            tooltip.add(Text.translatable("tier.travelersbackpack." + Tiers.of(stack.get(ModDataComponents.TIER)).getName()));
        }

        if(stack.contains(ModDataComponents.BACKPACK_CONTAINER) && !BackpackUtils.isCtrlPressed())
        {
