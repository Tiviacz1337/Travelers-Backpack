...
public class ServerboundSettingsPacket
{
    ...
    public static void handle(final ServerboundSettingsPacket message, final Supplier<NetworkEvent.Context> ctx)
    {
        ctx.get().enqueueWork(() -> {
            ...
            if(serverPlayer != null)
            {
                if(message.screenID == Reference.WEARABLE_SCREEN_ID)
                {
                    ...
                    //Reset slots
                    //((TravelersBackpackBaseMenu)serverPlayer.containerMenu).refreshAllSlots();
                }
