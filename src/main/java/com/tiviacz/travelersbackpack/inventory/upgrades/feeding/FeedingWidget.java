    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        if(isTabOpened() && isBackpackOwner()) {
            if(this.whitelistButton.mouseClicked(pMouseX, pMouseY, pButton)) {
                PacketDistributorHelper.sendToServer(new ServerboundFilterSettingsPacket(this.dataHolderSlot, List.of(whitelistButton.getCurrentState(), hungerModeButton.getCurrentState(), ignoreEffectModeButton.getCurrentState())));
