    ...
    public List<Slot> getUpgradeSlots(BackpackBaseMenu menu, BackpackWrapper wrapper, int x, int y) {
        ...
        slots.add(new SlotItemHandler(diskHandler, 0, x + 7, y + 23) {
            ...
            public boolean mayPlace(ItemStack pStack) {
                return isTabOpened() && !getUpgradeManager().getUpgradesHandler().getStackInSlot(getDataHolderSlot()).getOrDefault(ModDataComponents.IS_PLAYING.get(), false);
            }

            @Override
            public boolean mayPickup(Player playerIn) {
                return super.mayPickup(playerIn) && !getUpgradeManager().getUpgradesHandler().getStackInSlot(getDataHolderSlot()).getOrDefault(ModDataComponents.IS_PLAYING.get(), false);
            }
        });
        return slots;
    }
