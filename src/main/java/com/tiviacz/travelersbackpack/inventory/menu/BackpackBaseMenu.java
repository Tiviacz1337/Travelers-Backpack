...
public class BackpackBaseMenu extends AbstractBackpackMenu {
    ...
    public void updateSlotsPosition() {
        ...
        for(int i = BACKPACK_INV_START; i < BACKPACK_INV_END; i++) {
            if(this.slots.get(i).getClass().equals(BackpackSlotItemHandler.class)) {
                ...
                if(slot < getWrapper().getSlotsInRow() - 1) {
                    slot++;
                } else {
                    slot = 0;
                }
    ...
    public void addPlayerInventoryAndHotbar(Inventory inventory, int currentItemIndex) {
        ...
        for(int x = 0; x < 9; x++) {
            this.addSlot(new Slot(inventory, x, modifiedOffset + 8 + x * 18, getWrapper().getRows() * 18 + 7 + 83));
        }
    }
    ...
    public static void clearSlotsAndPlaySound(Player player, ItemStackHandler handler, int size, boolean forbiddenOnly) {
    ...
    public static boolean clearSlot(ItemStackHandler handler, Player player, int index, boolean forbiddenOnly) {
        boolean flag = forbiddenOnly ? !BackpackSlotItemHandler.isItemValid(handler.getStackInSlot(index)) : !handler.getStackInSlot(index).isEmpty();
        if(flag) {
