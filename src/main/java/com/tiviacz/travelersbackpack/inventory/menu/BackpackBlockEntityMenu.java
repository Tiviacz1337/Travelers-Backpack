...
public class BackpackBlockEntityMenu extends BackpackBaseMenu {
    ...
    public BackpackBlockEntityMenu(int windowID, Inventory inventory, ModScreenHandlerTypes.BlockEntityScreenData data) {
    ...
    private static BackpackWrapper getBlockEntity(Inventory inventory, ModScreenHandlerTypes.BlockEntityScreenData data) {
    ...
    public boolean stillValid(Player player) { //stillValid
        return this.access.evaluate((level, blockPos) -> level.getBlockState(blockPos).is(this.backpackBlock) && (player.getId() == this.infniteAccessUser || player.isWithinBlockInteractionRange(blockPos, 4.0)), true);
    }
