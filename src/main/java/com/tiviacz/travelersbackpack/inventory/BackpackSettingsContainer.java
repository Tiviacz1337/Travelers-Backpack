...
public class BackpackSettingsContainer implements MenuProvider, Nameable {
    ...
    public static void openSettings(ServerPlayer serverPlayerEntity, ItemStack stack, byte screenID) {
        if(!serverPlayerEntity.level().isClientSide) {
            serverPlayerEntity.openMenu(new BackpackSettingsContainer(stack, serverPlayerEntity, screenID), buf -> saveSettingsExtraData(new RegistryFriendlyByteBuf(buf, serverPlayerEntity.registryAccess()), screenID, stack));
        }
    }
}