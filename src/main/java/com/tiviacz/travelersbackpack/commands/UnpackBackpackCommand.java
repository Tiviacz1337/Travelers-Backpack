...
public class UnpackBackpackCommand
{
    ...
    public static int unpackTargetBlockEntity(ServerCommandSource source, BlockPos blockPos) throws CommandSyntaxException
    {
        if(source.getWorld().getBlockEntity(blockPos) instanceof TravelersBackpackBlockEntity blockEntity)
        {
            ...
            if(!stacks.isEmpty())
            {
                ...
                source.sendFeedback(new LiteralText("Dropping contents of backpack placed at " + blockPos.toShortString()), true);
                return 1;
            }
            else
            {
                source.sendError(new LiteralText("There's no contents in backpack at coordinates " + blockPos.toShortString()));
                return -1;
            }
        }
        else
    ...
    public static int unpackTargetInventory(ServerCommandSource source, ServerPlayerEntity serverPlayer) throws CommandSyntaxException
    {
        ...
        if(hasBackpack)
        {
            ...
            if(flag.get())
            {
                source.sendFeedback(new LiteralText("Dropping contents of " + serverPlayer.getDisplayName().getString() + " backpack at " + serverPlayer.getBlockPos().toShortString()), true);
                return 1;
            }
            else
            {
                source.sendError(new LiteralText("There's no contents in " + serverPlayer.getDisplayName().getString() + " backpack"));
                return -1;
            }
        }
        else
        {
            source.sendError(new LiteralText("Player " + serverPlayer.getDisplayName().getString() + " is not wearing backpack"));
            return -1;
        }
    }
}