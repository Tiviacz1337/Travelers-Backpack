...
public class RestoreBackpackCommand
{
    public RestoreBackpackCommand(CommandDispatcher<CommandSourceStack> dispatcher)
    {
        ...
        Command<CommandSourceStack> restore = (commandSource) -> {
            ...
            if(backpack == null)
            {
                commandSource.getSource().sendFailure(new TextComponent("Backpack with ID " + backpackID.toString() + " not found"));
                return 0;
            }
            ...
            commandSource.getSource().sendSuccess(new TextComponent("Successfully restored " + player.getDisplayName().getString() + "'s backpack"), true);
            return 1;
        };
