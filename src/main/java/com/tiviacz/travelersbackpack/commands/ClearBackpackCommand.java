...
public class ClearBackpackCommand
{
    ...
    private static int removeBackpack(CommandSourceStack source, ServerPlayer player) throws CommandSyntaxException
    {
        if(CapabilityUtils.isWearingBackpack(player))
        {
            ...
            source.sendSuccess(new TextComponent("Removed Traveler's Backpack from " + player.getDisplayName().getString()), true);
            return 1;
        }
        else
        {
            source.sendFailure(new TextComponent("Player " + player.getDisplayName().getString() + " is not wearing backpack"));
            return -1;
        }
    }
    ...
    private static int clearBackpack(CommandSourceStack source, ServerPlayer player) throws CommandSyntaxException
    {
        if(CapabilityUtils.isWearingBackpack(player))
        {
            ...
            source.sendSuccess(new TextComponent("Cleared contents of Traveler's Backpack from " + player.getDisplayName().getString()), true);
            return 1;
        }
        else
        {
