...
public class RenderUtils 
{
	...
	public static void renderGuiTank(FluidStack fluid, int capacity, int amount, double x, double y, double height, double width) 
	{
	    ...
	    for(int i = 0; i < width; i += 16) 
	    {
	    	for(int j = 0; j < renderAmount; j += 16) 
	    	{
	    		...
	    		tes.pos(drawX, drawY + drawHeight, 0).tex(minU, minV + (maxV - minV) * (float)drawHeight / 16F).endVertex();
	    		tes.pos(drawX + drawWidth, drawY + drawHeight, 0).tex(minU + (maxU - minU) * (float)drawWidth / 16F, minV + (maxV - minV) * drawHeight / 16F).endVertex();
	...
	public static void renderFluidSides(double height, FluidStack fluid)
	{
		...
		for(EnumFacing side : EnumFacing.VALUES) 
		{
			...
			worldRenderer.pos(c[0][0], getHeight(c[0][1], height), c[0][2]).tex(replacedU1, replacedMaxV).color(r, g, b, a).endVertex();
		    worldRenderer.pos(c[1][0], getHeight(c[1][1], height), c[1][2]).tex(replacedU1, icon.getMinV()).color(r, g, b, a).endVertex();
		    worldRenderer.pos(c[2][0], getHeight(c[2][1], height), c[2][2]).tex(replacedU2, icon.getMinV()).color(r, g, b, a).endVertex();
		    worldRenderer.pos(c[3][0], getHeight(c[3][1], height), c[3][2]).tex(replacedU2, replacedMaxV).color(r, g, b, a).endVertex();
	...
	public static void renderFluidInTank(FluidTank tank, double x, double y, double z)
	{
		...
		renderFluidContext(tank.getFluid(), x, y, z, fluid ->
		{
			double height = getTankFillRatio(tank) * 0.99D;
			RenderUtils.renderFluidSides(height, fluid);
		});
