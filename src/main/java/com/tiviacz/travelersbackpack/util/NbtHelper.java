...
public class NbtHelper {
    ...
    public static CompoundTag expandTag(ItemStack stack, String key, int defaultSize) {
        ...
        for(int i = 0; i < tagList.size(); ++i) {
            CompoundTag itemTags = tagList.getCompound(i); //#TODO add size
    ...
    public static FluidVariantWrapper deserializeRightFluidStack(CompoundTag tag) {
        return FluidVariantWrapper.parseOptional(tag.getCompound("rightFluidStack"));
    }

    public static CompoundTag serializeFluids(Fluids fluids) {
        CompoundTag tag = new CompoundTag();
        tag.put("leftFluidStack", fluids.leftFluidStack().saveOptional());
        tag.put("rightFluidStack", fluids.rightFluidStack().saveOptional());
        return tag;
    }
