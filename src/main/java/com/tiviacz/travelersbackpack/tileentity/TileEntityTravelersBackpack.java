...
public class TileEntityTravelersBackpack extends TileEntity implements IInventoryTravelersBackpack, ITickable
{
	private final NonNullList<ItemStack> inventory = NonNullList.withSize(Reference.INVENTORY_SIZE, ItemStack.EMPTY);
	private final NonNullList<ItemStack> craftingGrid = NonNullList.withSize(Reference.CRAFTING_GRID_SIZE, ItemStack.EMPTY);
	...
	public TileEntityTravelersBackpack() {}
	...
	private EntityPlayer getUsingPlayer()
	{
        ...
        if(!world.isRemote)
        {
            ...
            {
        		if(entityplayer.openContainer instanceof ContainerTravelersBackpack)
        		{
        			return entityplayer;
        		}
            }
	...
	public EnumFacing getBlockFacing(TileEntity tile)
	{
		...
		{
			return world.getBlockState(getPos()).getValue(BlockTravelersBackpack.FACING);
		}
