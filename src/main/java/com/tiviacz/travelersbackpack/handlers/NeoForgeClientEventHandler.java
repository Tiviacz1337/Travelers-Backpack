...
public class NeoForgeClientEventHandler {
    ...
    public static void renderBackpackIcon(ScreenEvent.Render.Post event) {
        ...
        if(Minecraft.getInstance().screen instanceof InventoryScreen screen && AttachmentUtils.getCapability(player).isPresent()) {
            if(AttachmentUtils.getCapability(player).resolve().get().hasBackpack() && TravelersBackpack.enableIntegration()) {
                ItemStack backpack = AttachmentUtils.getCapability(player).resolve().get().getBackpack();
            ...
            if(AttachmentUtils.getCapability(player).resolve().get().hasBackpack() && TravelersBackpack.enableIntegration()) {
                if(event.getMouseX() >= screen.getGuiLeft() + 77 && event.getMouseX() < screen.getGuiLeft() + 77 + 16 && event.getMouseY() >= screen.getGuiTop() + 62 - 18 && event.getMouseY() < screen.getGuiTop() + 62 - 18 + 16) {
                    if(event.getButton() == GLFW.GLFW_MOUSE_BUTTON_1) {
                        PacketDistributorHelper.sendToServer(new ServerboundRetrieveBackpackPacket(AttachmentUtils.getCapability(player).resolve().get().getBackpack().getItem().getDefaultInstance()));
                    }
                }
            }
        }
   ...
   /* @SubscribeEvent
    ...
    @SubscribeEvent
    public static void clientTickEvent(final TickEvent.ClientTickEvent.Pre event) {
        Player player = Minecraft.getInstance().player;
        if(player == null) return;
        ...
        if(TravelersBackpackConfig.CLIENT.disableScrollWheel.get()) {
            ...
            if(!ToolSlotItemHandler.isValid(heldItem)) {
                while(ModClientEventHandler.SWAP_TOOL.consumeClick()) {
                    if(!heldItem.isEmpty()) {
                        if(heldItem.getItem() instanceof HoseItem && heldItem.has(ModDataComponents.HOSE_MODES.get())) {
