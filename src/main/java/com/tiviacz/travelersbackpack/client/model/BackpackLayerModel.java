...
public class BackpackLayerModel<T extends LivingEntity> extends HumanoidModel<T> {
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLightIn, int packedOverlayIn, float r, float g, float b, float alpha) {
        ...
        if(!NbtHelper.getOrDefault(getBackpackStack(), ModDataHelper.RENDER_INFO, RenderInfo.EMPTY).isEmpty()) {
        ...
        this.mainBody.render(poseStack, vertexConsumer, packedLightIn, packedOverlayIn, r, g, b, alpha);
