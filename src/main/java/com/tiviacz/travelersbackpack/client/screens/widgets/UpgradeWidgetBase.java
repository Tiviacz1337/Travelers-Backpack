    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        ...
        if(isMouseOverIcon(pMouseX, pMouseY)) {
            if(this.upgrade.isTabOpened()) {
                PacketDistributorHelper.sendToServer(new ServerboundTabPacket(this.dataHolderSlot, false, ServerboundTabPacket.TAB_OPEN));
            } else {
                PacketDistributorHelper.sendToServer(new ServerboundTabPacket(this.dataHolderSlot, true, ServerboundTabPacket.TAB_OPEN));
            }
    ...
    public boolean removeButtonMouseClicked(double pMouseX, double pMouseY, int pButton) {
        if(this.upgrade.isTabOpened()) {
            if(isMouseOverRemoveButton(pMouseX, pMouseY)) {
                ...
                PacketDistributorHelper.sendToServer(new ServerboundRemoveUpgradePacket(this.dataHolderSlot));
                this.screen.playUIClickSound();
                return true;
            }
        }
    ...
    public boolean enableButtonMouseClicked(double pMouseX, double pMouseY, int pButton) {
        if(this.upgrade instanceof IEnable e && !this.upgrade.isTabOpened()) {
            if(isMouseOverEnableButton(pMouseX, pMouseY)) {
                ...
                PacketDistributorHelper.sendToServer(new ServerboundTabPacket(this.dataHolderSlot, !e.isEnabled(), ServerboundTabPacket.UPGRADE_ENABLED));
                this.screen.playUIClickSound();
                return true;
            }
        }
