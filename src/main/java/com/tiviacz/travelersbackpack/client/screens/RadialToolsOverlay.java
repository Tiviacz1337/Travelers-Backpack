            ...
            if(!hoveredStack.isEmpty()) {
                ...
                if(hoveredStack.getItem() instanceof BackpackTankItem) {
                    if(backpack.getOrDefault(ModDataComponents.RENDER_INFO, RenderInfo.EMPTY).hasTanks()) {
                        ...
                        if(hoveredResult == 2) {
                            FluidStacksResourceHandler rightTank = new FluidStacksResourceHandler(1, renderInfo.getCapacity());
                            StacksHandlerUtils.setFluid(rightTank, renderInfo.getRightFluidStack());
                            tooltip.addAll(TankWidget.getTankTooltip(rightTank));
                        }
                        if(hoveredResult == 3) {
                            FluidStacksResourceHandler leftTank = new FluidStacksResourceHandler(1, renderInfo.getCapacity());
                            StacksHandlerUtils.setFluid(leftTank, renderInfo.getLeftFluidStack());
