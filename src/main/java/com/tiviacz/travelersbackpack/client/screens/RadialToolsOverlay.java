...
public final class RadialToolsOverlay {
        ...
        if(hoveredIsPlus) {
            ...
            if(!hoveredStack.isEmpty()) {
                ...
                if(hoveredStack.getItem() instanceof BackpackTankItem) {
                    if(NbtHelper.getOrDefault(backpack, ModDataHelper.RENDER_INFO, RenderInfo.EMPTY).hasTanks()) {
                ...
                guiGraphics.renderTooltip(font, tooltip, hoveredStack.getTooltipImage(), mouseX, mouseY);
            }
        }
    ...
    private static void renderPlusButton(GuiGraphics g, Font font, int x, int y) {
        ...
        drawString(g, font, plus, px + 0.5F, py + 1.0F, 0xFFFFFF, false);

        g.pose().popPose();
    }

    public static int drawString(GuiGraphics guiGraphics, Font pFont, String pText, float pX, float pY, int pColor, boolean pDropShadow) {
        if (pText == null) {
            return 0;
        } else {
            int i = pFont.drawInBatch(pText, (float)pX, (float)pY, pColor, pDropShadow, guiGraphics.pose().last().pose(), guiGraphics.bufferSource(), Font.DisplayMode.NORMAL, 0, 15728880, pFont.isBidirectional());
