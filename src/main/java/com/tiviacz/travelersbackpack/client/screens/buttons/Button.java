...
public abstract class Button implements IButton {
    ...
    public void drawButton(GuiGraphics guiGraphics, int mouseX, int mouseY, ResourceLocation texture, int u1, int v1, int u2, int v2) {
        guiGraphics.blit(RenderType::guiTextured, texture, screen.getGuiLeft() + x + 1, screen.getGuiTop() + y + 1, u1, v1, width - 2, height - 2, 256, 256);
        if(this.inButton(mouseX, mouseY)) {
            guiGraphics.blit(RenderType::guiTextured, texture, screen.getGuiLeft() + x, screen.getGuiTop() + y, u2, v2, width, height, 256, 256);
        }
    }
