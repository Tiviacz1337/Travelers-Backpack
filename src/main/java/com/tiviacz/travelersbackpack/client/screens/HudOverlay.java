...
public class HudOverlay implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        if(TravelersBackpackConfig.CLIENT.overlay.enableOverlay.get() && !Minecraft.getInstance().options.hideGui && AttachmentUtils.isWearingBackpack(Minecraft.getInstance().player) && Minecraft.getInstance().gameMode.getPlayerMode() != GameType.SPECTATOR) {
            ...
            if(stack.has(ModDataComponents.TOOLS_CONTAINER.get())) {
                ...
                NonNullList<ItemStack> tools = getTools(stack.get(ModDataComponents.TOOLS_CONTAINER.get()).getItems());
