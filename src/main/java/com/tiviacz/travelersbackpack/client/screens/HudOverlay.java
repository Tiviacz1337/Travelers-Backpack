    ...
    public static void renderOverlay(ItemStack stack, Minecraft mc, GuiGraphics guiGraphics) {
        ...
        if(!stack.getOrDefault(ModDataComponents.RENDER_INFO, RenderInfo.EMPTY).isEmpty()) {
            ...
            if(player != null && player.getMainHandItem().getItem() instanceof HoseItem) {
                ...
                if(tank == 1) {
                    guiGraphics.blit(RenderType::guiTextured, OVERLAY, scaledWidth, scaledHeight, textureX, textureY, 10, 23, 256, 256);
                    guiGraphics.blit(RenderType::guiTextured, OVERLAY, scaledWidth - 12, scaledHeight, selectedTextureX, selectedTextureY, 10, 23, 256, 256);
                }

                if(tank == 2) {
                    guiGraphics.blit(RenderType::guiTextured, OVERLAY, scaledWidth, scaledHeight, selectedTextureX, selectedTextureY, 10, 23, 256, 256);
                    guiGraphics.blit(RenderType::guiTextured, OVERLAY, scaledWidth - 12, scaledHeight, textureX, textureY, 10, 23, 256, 256);
                }
