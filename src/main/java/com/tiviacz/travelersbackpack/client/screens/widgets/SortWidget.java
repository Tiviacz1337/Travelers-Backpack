...
public class SortWidget extends WidgetBase
{
    ...
    public void renderTooltip(PoseStack poseStack, int mouseX, int mouseY)
    {
        if(isHovered && showTooltip)
        {
            ...
            component.add(new TextComponent(s[0]));
            component.add(new TextComponent(s[1]));

            if(mouseX >= x + 1 && mouseY >= y + 15 && mouseX < x + 11 && mouseY < y + 25)
            {
                component.add(new TranslatableComponent("screen.travelersbackpack.select_all"));
            }
            if(mouseX >= x + 13 && mouseY >= y + 15 && mouseX < x + 23 && mouseY < y + 25)
            {
                component.add(new TranslatableComponent("screen.travelersbackpack.remove_all"));
            }
