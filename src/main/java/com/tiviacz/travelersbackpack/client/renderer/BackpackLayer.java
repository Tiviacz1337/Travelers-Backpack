...
public class BackpackLayer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    private static void renderModel(PoseStack matrixStack, BakedModel model, int packedLightIn) {
        ...
        for(BakedQuad quad : quads) {
            worldrenderer.putBulkData(matrixStack.last(), quad, new float[]{1.0F, 1.0F, 1.0F, 1.0F}, 1.0f, 1.0f, 1.0f, 1.0f, new int[]{packedLightIn, packedLightIn, packedLightIn, packedLightIn}, OverlayTexture.NO_OVERLAY, true);
        }
        src.endBatch();
    }
}