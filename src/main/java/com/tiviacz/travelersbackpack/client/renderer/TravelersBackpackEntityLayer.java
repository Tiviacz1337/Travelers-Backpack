...
public class TravelersBackpackEntityLayer extends RenderLayer<LivingEntity, HumanoidModel<LivingEntity>>
{
    ...
    {
        if (pLivingEntity.getItemBySlot(EquipmentSlot.CHEST).getItem() instanceof TravelersBackpackItem) {
            TravelersBackpackLayer.renderBackpackLayer(BackpackLayerModel.LAYER_MODEL, getParentModel(), pPoseStack, pBuffer, pPackedLight, pLivingEntity, pLivingEntity.getItemBySlot(EquipmentSlot.CHEST), pLimbSwing, pLimbSwingAmount, pPartialTick, pAgeInTicks, pNetHeadYaw, pHeadPitch);
        }
    }
}