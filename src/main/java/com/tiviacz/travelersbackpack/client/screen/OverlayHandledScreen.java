    ...
    public void renderOverlay(MatrixStack matrices)
    {
        ...
        if(inv.getTier() != null)
        {
            ...
            if(key.isPressed() && tools.size() > 2)
            {
                ...
                {
                    drawItemStack(matrices, getTools(inv.getTier(), inv.getInventory()).get(i), scaledWidth - 30, (int)(scaledHeight + 11 - (animationProgress * (i * 15))));
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        drawItemStack(matrices, inv.getInventory().getStack(inv.getTier().getSlotIndex(Tiers.SlotType.TOOL_FIRST)), scaledWidth - 30, scaledHeight - 4);
                    }
                    if(tools.size() > 1)
                    {
                        if(!inv.getInventory().getStack(inv.getTier().getSlotIndex(Tiers.SlotType.TOOL_SECOND) + tools.size() - 2).isEmpty())
                        {
                            drawItemStack(matrices, inv.getInventory().getStack(inv.getTier().getSlotIndex(Tiers.SlotType.TOOL_SECOND)  + tools.size() - 2), scaledWidth - 30, scaledHeight + 11);
                        }
                    }
                }
            }
