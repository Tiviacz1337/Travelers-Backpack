    ...
    public void drawMouseoverTooltip(MatrixStack matrixStack, int mouseX, int mouseY)
    {
        if(isHovered && showTooltip && isVisible)
        {
            ...
            else
            {
                ...
                {
                    screen.renderTooltip(matrixStack, Arrays.asList(new TranslatableText("screen.travelersbackpack.crafting_lock"), new TranslatableText("screen.travelersbackpack.crafting_lock_description")), mouseX, mouseY);
                }

                if(in(mouseX, mouseY, x, y + 27, 12, 11))
                {
                    screen.renderTooltip(matrixStack, Arrays.asList(new TranslatableText("screen.travelersbackpack.crafting_overlay"), new TranslatableText("screen.travelersbackpack.crafting_overlay_description")), mouseX, mouseY);
                }
            }
        }
    }
