            ...
            if(BackpackAbilities.isOnList(screen.inventory.getScreenID() == Reference.WEARABLE_SCREEN_ID ? BackpackAbilities.ITEM_ABILITIES_LIST : BackpackAbilities.BLOCK_ABILITIES_LIST, screen.inventory.getItemStack()) && this.inButton(mouseX, mouseY) && !screen.isWidgetVisible(3, screen.leftTankSlotWidget) && !screen.isWidgetVisible(4, screen.leftTankSlotWidget))
            {
                if(!screen.toolSlotsWidget.isCoveringAbility())
                {
                    if(screen.inventory.getAbilityValue())
                    {
                        ...
                        {
                            list.add(screen.inventory.getLastTime() == 0 ? Text.translatable("screen.travelersbackpack.ability_ready") : Text.translatable(BackpackUtils.getConvertedTime(screen.inventory.getLastTime())));
                        }
                        context.drawTooltip(screen.getTextRenderer(), list, mouseX, mouseY);
                    }
                    else
                    {
                        if(!TravelersBackpackConfig.getConfig().backpackAbilities.enableBackpackAbilities || !BackpackAbilities.ALLOWED_ABILITIES.contains(screen.inventory.getItemStack().getItem()))
                        {
                            context.drawTooltip(screen.getTextRenderer(), Text.translatable("screen.travelersbackpack.ability_disabled_config"), mouseX, mouseY);
                        }
                        else
                        {
                            context.drawTooltip(screen.getTextRenderer(), Text.translatable("screen.travelersbackpack.ability_disabled"), mouseX, mouseY);
                        }
                    }
                }
            }
