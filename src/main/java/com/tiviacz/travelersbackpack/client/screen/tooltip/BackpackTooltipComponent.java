...
public class BackpackTooltipComponent implements TooltipComponent
{
    ...
    public void drawItems(TextRenderer textRenderer, int x, int y, MatrixStack matrices, ItemRenderer itemRenderer)
    {
        ...
        if(BackpackUtils.isCtrlPressed())
        {
            ...
            if(!component.inventory.isEmpty())
            {
                for(int j = 0; j < 7; j++)
                {
                    for(int i = 0; i < 9; i++)
                    {
                        ...
                        int j1 = y + j * 18 + 6;
                        this.renderItemInSlot(i1, j1, slot, textRenderer, matrices, itemRenderer, false);
                        slot++;
                    }
                }
            }
            ...
            if(!component.craftingInventory.isEmpty())
            {
                for(int j = 0; j < 3; j++)
                {
                    for(int i = 0; i < 3; i++)
                    {
                        ...
                        this.renderItemInSlot(i1, j1, craftingSlot, textRenderer, matrices, itemRenderer, true);
                        craftingSlot++;
                    }
                }
            }
            ...
            {
                this.renderItemInSlot(x + 42 - 18, y + (component.tier.getOrdinal() * 18) + 18 + 6, slot, textRenderer, matrices, itemRenderer, false);
                slot++;
            }

            if(component.hasToolInSlot(Tiers.SlotType.TOOL_UPPER))
            {
                this.renderItemInSlot(x + 42 - 18, y + (component.tier.getOrdinal() * 18) + 36 + 6, slot, textRenderer, matrices, itemRenderer, false);
                slot++;
            }
    ...
    private void renderItemInSlot(int x, int y, int slot, TextRenderer textRenderer, MatrixStack matrices, ItemRenderer itemRenderer, boolean isCrafting)
    {
        ...
        itemRenderer.renderGuiItemIcon(matrices, stack, x + 1, y + 1);
        itemRenderer.renderGuiItemOverlay(matrices, textRenderer, stack, x + 1, y + 1);
    }

    private void blit(MatrixStack matrices, int x, int y)
    {
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        RenderSystem.setShaderTexture(0, getTooltipTexture());
        Screen.drawTexture(matrices, x, y, 0, 0, 211, getTextureHeight(), 256, 256);
    }
