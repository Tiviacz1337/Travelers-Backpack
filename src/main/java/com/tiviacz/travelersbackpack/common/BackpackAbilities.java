...
public class BackpackAbilities
{
    ...
    public static void melonAbility(TravelersBackpackBlockEntity blockEntity)
    {
        ...
        {
            Block.dropStack(blockEntity.getWorld(), blockEntity.getPos(), new ItemStack(Items.MELON_SLICE, blockEntity.getWorld().random.nextInt(0, 3)));
            blockEntity.setLastTime(blockEntity.getWorld().random.nextInt(120 * 20, 480 * 20));
    ...
    public void cowAbility(PlayerEntity player)
    {
        if(!player.getActiveStatusEffects().isEmpty() && ComponentUtils.getBackpackInv(player).getLastTime() <= 0)
        {
            ...
            if(!player.world.isClient)
            {
                player.world.syncWorldEvent(2007, player.getLandingPos(), 16777215);
            }
            ...
            ComponentUtils.getBackpackInv(player).setLastTime(player.world.random.nextInt(450 * 20, 600 * 20));
