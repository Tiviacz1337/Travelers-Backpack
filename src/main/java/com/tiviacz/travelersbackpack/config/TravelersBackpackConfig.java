...
{
    //Compat only, Use values from getConfig() instead
    ...
    public static void register()
    {
        ...
        ServerLifecycleEvents.START_DATA_PACK_RELOAD.register((s, m) ->
        {
            ...
            //Compat only
            backpackDeathPlace = getConfig().backpackSettings.backpackDeathPlace;
            trinketsIntegration = getConfig().backpackSettings.trinketsIntegration;
        });
    }
    ...
    {
        return Arrays.stream(list).anyMatch(p -> p.equals(Registry.ENTITY_TYPE.getId(value.getType()).toString()));
    }

    public static boolean isOnItemList(ItemStack value, String[] list)
    {
        return Arrays.stream(list).anyMatch(p -> p.equals(Registry.ITEM.getId(value.getItem()).toString()));
    }
    ...
    public static Item getRandomCompatibleOverworldBackpackEntry(Random random)
    {
        ...
        return Registry.ITEM.getOrEmpty(Identifier.tryParse(selectedBackpack)).orElseThrow(() -> new NoSuchElementException("Wrong backpack registry name specified in the config!"));
    }
    ...
    public static TravelersBackpackConfigData readFromNbt(NbtCompound nbt)
    {
        ...
        //Compat
        TravelersBackpackConfig.backpackDeathPlace = data.backpackSettings.backpackDeathPlace;
        TravelersBackpackConfig.trinketsIntegration = data.backpackSettings.trinketsIntegration;
