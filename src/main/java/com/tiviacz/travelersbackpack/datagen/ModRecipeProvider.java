...
public class ModRecipeProvider extends RecipeProvider
{
    ...
    protected void buildCraftingRecipes(Consumer<FinishedRecipe> writer)
    {
        ...
        {
            BackpackUpgradeRecipeBuilder.upgrade(Ingredient.of(createTieredStack(item, Tiers.LEATHER)), Ingredient.of(ModItems.IRON_TIER_UPGRADE.get()), item).unlocks("has_iron_tier_upgrade", RecipeProvider.has(ModItems.IRON_TIER_UPGRADE.get())).save(writer, Registry.ITEM.getKey(item.asItem()).getPath() + "_smithing_iron");
            BackpackUpgradeRecipeBuilder.upgrade(Ingredient.of(createTieredStack(item, Tiers.IRON)), Ingredient.of(ModItems.GOLD_TIER_UPGRADE.get()), item).unlocks("has_gold_tier_upgrade", RecipeProvider.has(ModItems.GOLD_TIER_UPGRADE.get())).save(writer, Registry.ITEM.getKey(item.asItem()).getPath() + "_smithing_gold");
            BackpackUpgradeRecipeBuilder.upgrade(Ingredient.of(createTieredStack(item, Tiers.GOLD)), Ingredient.of(ModItems.DIAMOND_TIER_UPGRADE.get()), item).unlocks("has_diamond_tier_upgrade", RecipeProvider.has(ModItems.DIAMOND_TIER_UPGRADE.get())).save(writer, Registry.ITEM.getKey(item.asItem()).getPath() + "_smithing_diamond");
            BackpackUpgradeRecipeBuilder.upgrade(Ingredient.of(createTieredStack(item, Tiers.DIAMOND)), Ingredient.of(ModItems.NETHERITE_TIER_UPGRADE.get()), item).unlocks("has_netherite_tier_upgrade", RecipeProvider.has(ModItems.NETHERITE_TIER_UPGRADE.get())).save(writer, Registry.ITEM.getKey(item.asItem()).getPath() + "_smithing_netherite");
        }
    }
