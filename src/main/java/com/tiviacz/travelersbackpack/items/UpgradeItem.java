...
public class UpgradeItem extends Item
{
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag)
    {
        if(TravelersBackpackConfig.enableTierUpgrades)
        {
            switch(type)
            {
                ...
                case IRON_TIER_UPGRADE:
                    tooltip.add(new TranslatableComponent("item.travelersbackpack.tier_upgrade_tooltip", Tiers.LEATHER.getName()).withStyle(ChatFormatting.BLUE));
                    break;
                case GOLD_TIER_UPGRADE:
                    tooltip.add(new TranslatableComponent("item.travelersbackpack.tier_upgrade_tooltip", Tiers.IRON.getName()).withStyle(ChatFormatting.BLUE));
                    break;
                case DIAMOND_TIER_UPGRADE:
                    tooltip.add(new TranslatableComponent("item.travelersbackpack.tier_upgrade_tooltip", Tiers.GOLD.getName()).withStyle(ChatFormatting.BLUE));
                    break;
                case NETHERITE_TIER_UPGRADE:
                    tooltip.add(new TranslatableComponent("item.travelersbackpack.tier_upgrade_tooltip", Tiers.DIAMOND.getName()).withStyle(ChatFormatting.BLUE));
                    break;
            }
        }
        else
        {
            addTooltipForUpgrades(tooltip, new TranslatableComponent("item.travelersbackpack.upgrade_disabled"),
        ...
        if(type == Upgrade.CRAFTING_UPGRADE)
        {
            ...
            {
                tooltip.add(new TranslatableComponent("item.travelersbackpack.crafting_upgrade_tooltip").withStyle(ChatFormatting.BLUE));
            }
            else
            {
                tooltip.add(new TranslatableComponent("item.travelersbackpack.upgrade_disabled").withStyle(ChatFormatting.RED));
            }
        }
    }
