...
public class ItemHose extends ItemBase
{
	...
	public FluidTank getSelectedFluidTank(ItemStack stack, InventoryTravellersBackpack inv)
	{
						...
						if(nonFluidResult != null && nonFluidResult.typeOfHit == RayTraceResult.Type.BLOCK)
						{
							...
							if(tank.getFluid() != null && tank.getFluidAmount() > 0)
							{
								...
								if(fluidStack.getFluid().canBePlacedInWorld())
								{
									...
									if(worldIn.provider.isNether() && fluidStack.getFluid() == FluidRegistry.WATER)
									{
									...
									else
									{
										...
										if(drainedFluid != null && drainedFluid.amount >= Reference.BUCKET)
										{
											...
											{
												if(worldIn.setBlockState(newResult, Blocks.FLOWING_WATER.getDefaultState()))
												{
													tank.drain(Reference.BUCKET, true);
												}
											} 
											...
											{
												if(worldIn.setBlockState(newResult, Blocks.FLOWING_LAVA.getDefaultState()))
												{
													tank.drain(Reference.BUCKET, true);
												}
											}
											
											else if(worldIn.setBlockState(newResult, fluidStack.getFluid().getBlock().getDefaultState()))
											{
												tank.drain(Reference.BUCKET, true);
											}
