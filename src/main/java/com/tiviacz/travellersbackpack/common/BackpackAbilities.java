...
import com.tiviacz.travellersbackpack.network.client.ParticlesPacket;
...
public class BackpackAbilities 
{
    ...
    public void executeItemAbility(EntityPlayer player, World world, ItemStack stack)
    {
    		...
    		{
    			TravellersBackpack.NETWORK.sendToAll(new ParticlesPacket((byte)2, player.getEntityId()));
    		}
    ...
    public void executeTileAbility(EntityPlayer player, World world, TileEntityTravellersBackpack tile)
    {
	...
	public void itemRainbow(EntityPlayer player, World world, ItemStack stack)
	{
     ...
     /*   if(noteTime >= 0 && noteTime < (world.rand.nextInt(20) + 10) * 20)
        {
            ...
            if(noteTime % 2 == 0)
            {
            	TravellersBackpack.NETWORK.sendToAll(new ParticlesPacket((byte)0, player.getEntityId()));
            }
            ...
            if(noteTime == 0)
            {
            	...
            	noteTime = (world.rand.nextInt(100) + 60) * 20;
            }
        }
        else
        {
        	player.addPotionEffect(new PotionEffect(MobEffects.SPEED, 210, 0, false, false));
        } */
        ...
        if(player.onGround)
        {
            ...
            if(player.isSprinting())
            {
                ...
                if(slimeTime <= 0)
                {
                    if(!world.isRemote)
                    {
                        TravellersBackpack.NETWORK.sendToAll(new ParticlesPacket((byte)1, player.getEntityId()));
                    }
	...
	public void tileCactus(EntityPlayer player, World world, TileEntityTravellersBackpack tile) 
	{
